import React, { useState } from 'react';
import { BaseModal } from './BaseModal';
import { LogisticsDetails } from '../../types';
import { COURIER_OPTIONS, FILE_TYPES } from '../../utils/constants';

interface LogisticsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: LogisticsDetails) => void;
  initialData?: LogisticsDetails;
}

export const LogisticsModal: React.FC<LogisticsModalProps> = ({
  isOpen,
  onClose,
  onSave,
  initialData,
}) => {
  const [date, setDate] = useState(initialData?.date || '');
  const [courierName, setCourierName] = useState(initialData?.courierName || '');
  const [docketNumber, setDocketNumber] = useState(initialData?.docketNumber || '');
  const [documents, setDocuments] = useState<File[]>([]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSave({
      date,
      courierName,
      docketNumber,
      documents,
    });
  };

  return (
    <BaseModal isOpen={isOpen} onClose={onClose} title="Logistics Details">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700">Date</label>
          <input
            type="date"
            value={date}
            onChange={(e) => setDate(e.target.value)}
            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
            required
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700">Courier Name</label>
          <select
            value={courierName}
            onChange={(e) => setCourierName(e.target.value)}
            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
            required
          >
            <option value="">Select Courier</option>
            {COURIER_OPTIONS.map(option => (
              <option key={option.value} value={option.value}>
                {option.label}
              </option>
            ))}
          </select>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700">Docket Number</label>
          <input
            type="text"
            value={docketNumber}
            onChange={(e) => setDocketNumber(e.target.value)}
            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
            required
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700">Documents</label>
          <input
            type="file"
            onChange={(e) => {
              if (e.target.files) {
                setDocuments(Array.from(e.target.files));
              }
            }}
            className="mt-1 block w-full"
            multiple
            accept={FILE_TYPES.ACCEPTED_FORMATS}
          />
        </div>
        <div className="flex justify-end gap-3 mt-6">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50"
          >
            Cancel
          </button>
          <button
            type="submit"
            className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700"
          >
            Save
          </button>
        </div>
      </form>
    </BaseModal>
  );
};